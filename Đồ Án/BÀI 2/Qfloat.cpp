...
//Hàm làm tròn số thực lớn
//input: số thực lớn dạng chuỗi, số chữ số sau dấu chấm(n>0). Nếu n=-1 thì không làm tròn.
//outPut: chuỗi số đã làm tròn
string Qfloat::LamTron(string bigNum, int n)
{
	if (n >= 0)
	{
		int indexDot = bigNum.find('.');
		if (indexDot > 0 && indexDot < bigNum.length() && indexDot + n < bigNum.length())
		{
			string a = "0.";
			for (int i = 0; i < n - 1; i++)
				a += '0';
			a += '1';
			if (bigNum[indexDot + n + 1] >= '5')
			{
				bigNum.erase(indexDot + n+1, bigNum.length() - indexDot - n-1);
				bigNum = addFloat(bigNum, a);
			}
			else
				bigNum.erase(indexDot + n +1, bigNum.length() - indexDot - n-1);
			if (bigNum[bigNum.length() - 1] == '.')
				bigNum.erase(bigNum.length() - 1, 1);
		}
